...
async function display_data_based_on_logged_status() {
  ...
  //If user is logged to a wallet
    ...
    document.getElementById("wallet_truncated").textContent = get_trunc_address(
...
async function refresh_platform_statistics() {
  ...
  let orgaNb = await contractInstance.methods
    .orgaCounter()
    .call({ from: currentUser.attributes.ethAddress });
  document.getElementById("orgaNb").textContent = orgaNb;
  let memberNb = await contractInstance.methods
    .memberCounter()
    .call({ from: currentUser.attributes.ethAddress });
...
async function create_orga() {
  ...
  let orga_membership_criteria = document.getElementById(
    "orga_membership_criteria"
  ).value;
  let orga_revokation_criteria = document.getElementById(
  ...
  contractInstance.methods
    ...
    .send({ from: currentUser.attributes.ethAddress })
...
async function change_active_tab(selected_tab) {
  ...
  if (selected_tab == "tab_my_memberships") {
    ...
    let memberships_orga_ids = await contractInstance.methods
      .memberOf(currentUser.attributes.ethAddress)
      .call({ from: currentUser.attributes.ethAddress });
    ...
    memberships_orga_ids.forEach(async (element) => {
      await contractInstance.methods
        ._organizations(element)
        .call({ from: currentUser.attributes.ethAddress })
        .then((receipt) => {
          addRowToTable("my_memberships", [
            receipt.name,
            receipt.logoURI,
    ...
    let myManagedOrganizations = await Moralis.Cloud.run(
    ...
    myManagedOrganizations.forEach(async (orga) => {
      await contractInstance.methods
        ._organizations(orga.attributes.orgaId)
        .call({ from: currentUser.attributes.ethAddress })
        .then((receipt) => {
          // create tempRow
          let tableRow = document.createElement("tr");
          ...
          document
            .getElementById("my_managed_organizations")
...
function get_added_members_minus_revoked_ones(added_members, revoked_members) {
  ...
  for (var i = added_members.length - 1; i > -1; i--) {
    if (
      !already_treated_added_members.has(
        added_members[i].attributes.addedMember
      )
    ) {
      already_treated_added_members.add(
        added_members[i].attributes.addedMember
      );
  ...
  for (i = revoked_members.length - 1; i > -1; i--) {
    if (
      !already_treated_revoked_members.has(
        revoked_members[i].attributes.addedMember
      )
    ) {
      already_treated_revoked_members.add(
        revoked_members[i].attributes.addedMember
      );
  ...
  for (i = 0; i < added_without_duplicates.length; i++) {
    ...
    for (let j = 0; j < revoked_without_duplicates.length; j++) {
      console.log(
        added_without_duplicates[i].attributes.addedMember +
          " " +
          revoked_without_duplicates[j].attributes.addedMember
      );
      if (
        added_without_duplicates[i].attributes.addedMember ==
        revoked_without_duplicates[j].attributes.addedMember
        ...
        console.log(
          new Date(added_without_duplicates[i].createdAt) +
            " " +
            new Date(revoked_without_duplicates[j].createdAt)
        );
        if (
          new Date(added_without_duplicates[i].createdAt) <
          new Date(revoked_without_duplicates[j].createdAt)
...
async function display_my_organizations_details(orgaId) {
  ...
  let addedMembers = await Moralis.Cloud.run("get_added_members_for_id", {
    orgaId: currentSelectedOrgaId,
  });
  let revokedMembers = await Moralis.Cloud.run("get_revoked_members_for_id", {
    orgaId: currentSelectedOrgaId,
  });

  let added_members_minus_revoked_ones = get_added_members_minus_revoked_ones(
    addedMembers,
    revokedMembers
  );
  ...
  added_members_minus_revoked_ones.forEach((member) => {
    ...
    let tableRow = document.createElement("tr");
    // add a column for each value
    let data = [member.attributes.addedMember];
    data.forEach((element) => {
      let newColumn = document.createElement("td");
      newColumn.innerHTML = element;
      tableRow.appendChild(newColumn);
...
async function revoke_member(orgaId, member_address) {
  ...
  contractInstance.methods
    .revokeMember(orgaId, member_address)
    .send({ from: currentUser.attributes.ethAddress })
    .on("receipt", function (receipt) {
      refresh_platform_statistics();
...
async function check_wallet() {
  ...
  memberships_orga_ids.forEach(async (element) => {
    await contractInstance.methods
      ...
      .call({ from: currentUser.attributes.ethAddress })
      .then((receipt) => {
        addRowToTable("checked_wallet_memberships", [
          receipt.name,
          receipt.logoURI,
          receipt.certificationURI,
          receipt.membershipCriteria,
...
async function add_member() {
  ...
  contractInstance.methods
    .addMember(currentSelectedOrgaId, newMemberWallet)
    .send({ from: currentUser.attributes.ethAddress })
...
async function check_organization() {
  ...
  await contractInstance.methods
    ._organizations(checked_orga_id)
    .call({ from: currentUser.attributes.ethAddress })
    .then((receipt) => {
      addRowToTable("checked_organization", [
        receipt.name,
        receipt.logoURI,
        receipt.certificationURI,
        receipt.membershipCriteria,
  ...
  let recorded_modifications = await Moralis.Cloud.run(
    "get_organization_modifications_for_id",
    { orgaId: checked_orga_id }
  );

  recorded_modifications.forEach((row) => {
    addRowToTable("orga_modifications_history", [
...
async function edit_orga_name() {
  ...
  await contractInstance.methods
    .changeName(currentSelectedOrgaId, newValue)
    .send({ from: currentUser.attributes.ethAddress })
    .then(() => {
...
async function edit_orga_logo_uri() {
  ...
  await contractInstance.methods
    .changeLogoURI(currentSelectedOrgaId, newValue)
    .send({ from: currentUser.attributes.ethAddress })
    .then(() => {
      change_active_tab("tab_my_organizations");
    });
...
async function edit_orga_certif_uri() {
  ...
  await contractInstance.methods
    .changeCertificationURI(currentSelectedOrgaId, newValue)
    .send({ from: currentUser.attributes.ethAddress })
    .then(() => {
      change_active_tab("tab_my_organizations");
    });
...
async function edit_orga_membership_criteria() {
  ...
  await contractInstance.methods
    .changeMembershipCriteria(currentSelectedOrgaId, newValue)
    .send({ from: currentUser.attributes.ethAddress })
    .then(() => {
      change_active_tab("tab_my_organizations");
    });
...
async function edit_orga_revokation_criteria() {
  ...
  await contractInstance.methods
    .changeRevokationCriteria(currentSelectedOrgaId, newValue)
    .send({ from: currentUser.attributes.ethAddress })
    .then(() => {
      change_active_tab("tab_my_organizations");
    });
...
document.getElementById("create_orga").onclick = function () {
  create_orga();
};
document.getElementById("check_wallet").onclick = function () {
  check_wallet();
};
document.getElementById("button_new_member").onclick = function () {
  add_member();
};
document.getElementById("check_orga_id").onclick = function () {
  check_organization();
};
document.getElementById("button_back_to_my_orgas").onclick = function () {
  on_click_button_back_to_my_organizations();
};
document.getElementById("button_edit_name").onclick = function () {
  edit_orga_name();
};
document.getElementById("button_edit_logo_uri").onclick = function () {
  edit_orga_logo_uri();
};
document.getElementById("button_edit_certif_uri").onclick = function () {
  edit_orga_certif_uri();
};
document.getElementById("button_edit_membership_criteria").onclick =
  function () {
    edit_orga_membership_criteria();
  };
document.getElementById("button_edit_revokation_criteria").onclick =
  function () {
    edit_orga_revokation_criteria();
  };

document.getElementById("title_tab_my_memberships").onclick = function () {
  change_active_tab("tab_my_memberships");
};
document.getElementById("title_tab_my_organizations").onclick = function () {
  change_active_tab("tab_my_organizations");
};
document.getElementById("title_tab_new_organization").onclick = function () {
  change_active_tab("tab_new_organization");
};
document.getElementById("title_tab_check_wallet").onclick = function () {
  change_active_tab("tab_check_wallet");
};
document.getElementById("title_tab_check_organization").onclick = function () {
  change_active_tab("tab_check_organization");
};
